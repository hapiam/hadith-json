...
### Update — book/chapter assignment specifically is RESOLVED, no re-scrape needed

Of the 13 corrupted fields, `bookNumber`/`bookName` (the book/chapter
assignment question) turned out not to need re-scraping at all.
`chapter.sectionNum` — captured by `_chapterRe`, a server-rendered-HTML
regex completely separate from the broken embedded-JSON anchor — already
holds amrayn's real book number, for every book *except* Malik (which has
its own always-reliable `malikChapterNum` instead, sourced from the
dedicated Malik scraper's own loop counter, never touched by this bug).
Verified against 9 live-refetched samples across the full span of
Bukhari (citations 300/500/1000/2000/3000/4000/5000/6000/6791) — all 9
matched `chapter.sectionNum` exactly.

| Book | Already resolved via existing data | Genuinely unresolved |
|---|---:|---:|
| Bukhari | 6,223 / 6,791 (91.6%) | 568 |
| Muslim | 8,592 / 8,898 (96.6%) | 306 |
| Nasa'i | 5,619 / 5,728 (98.1%) | 109 |
| Abu Dawud | 5,204 / 5,274 (98.7%) | 70 |
| Malik | 1,405 / 1,405 (100.0%) | 0 |
| **Total** | — | **1,053** (not 26,758) |

Book *names* (not just numbers) needed zero amrayn fetches too — this
repo's own `db/unified/catalog.json` (`bookChapters`) already has real
English/Arabic names for the same traditional numbering, built from
fawaz independently of amrayn.

Generated by `tool/build_amrayn_book_lookup.dart` (zero network
requests, reads only already-local files) into
`sources/amrayn.com/{book}_book_lookup.json` —
`{citation: {book, nameEn, nameAr, source}}` for every resolved citation.

**Still genuinely unresolved and still requiring a live re-fetch if ever
needed**: the other 12 corrupted fields (`chain`, `links`, `notes`,
`tags`, `gradeFlag`, `references`, `intlRef`, `rawPayload`, etc.) for the
full 26,758 records — `*.needs-rescrape.json` (below) is still the
accurate list for *those* fields specifically, just no longer relevant to
the book/chapter question.
