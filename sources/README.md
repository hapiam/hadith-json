...
| **sunnah.com** | ➖ not a scripted scrape — fetched directly via browser (WebFetch is blocked, 403) for a small number of pages where hadithunlocked/amrayn/fawaz all needed independent verification or didn't cover the content at all | ✅ `sources/sunnah.com/muslim_introduction/muslim_introduction_sunnah.json` (full text of Sahih Muslim's real Introduction chapter: 8 bab headers, 91 individually-referenced narrations, 5 prose essays — sunnah.com's own structure, not reconstructed) | Ground truth for Sahih Muslim's true Introduction (citations 1-7 plus 83 non-citable-but-real "Introduction N" narrations and Imam Muslim's own muqaddimah essays), replacing hadithunlocked's mis-chaptered/partly-wrong version — see `../DUPLICATE_HADITH_INVESTIGATION.md`'s Muslim section |
| **hadithunlocked.com** | ✅ `tool/import_hadithunlocked.dart` (not under `sources/`, predates this convention — imports a browser-downloaded export, not a live scrape) | ⚠️ raw export gitignored (`sources/hadithunlocked.com/original_source/`, 1.8GB, regenerate by re-downloading); merged result in `db/by_book/hadithunlocked/*.json` | 13 books (muslim, nasai-kubra, lulu-marjan, ibnrajab50, ibnhibban, bayhaqi, tabarani, hakim, ahmad-zuhd, daraqutni, bazzar, suyuti, ibnkhuzaymah). **Known defects (fixed 2026-07-29)**: (1) Tabarani's raw `item.number` field had ~340 duplicate numbers (already flagged in the import script's own doc comment) that turned out to be real content-level duplicate rows — a scraper-retry pattern; 38 exact duplicates removed, 187 quality-variance pairs still open (`TABARANI_DUPLICATE_TRANSLATIONS_REVIEW.md`); (2) Lu'lu' wal-Marjan had 69 duplicate rows genuinely present on hadithunlocked.com's own live page, not a scrape artifact (confirmed by fetching the page directly) — all 69 removed; (3) Muslim replaced fawaz as its book entirely — added 2026-07-29 to fix a numbering scheme fawaz never tracked correctly (see the Muslim-specific writeup below); (4) **hadithunlocked's own AI-translated English can fabricate content, not just mistranslate it** — user-flagged 2026-07-30: al-Sunan al-Kabir, Bayhaqi 14109 (`isAiTranslated: true`) had an English `text` field that didn't match its own Arabic at all — invented narrators/attribution not in the real isnad, and asserted a claim that's the literal opposite of both the real Arabic (Ibn 'Abbas's tafsir restricting Qur'an 2:223 to vaginal intercourse only, position notwithstanding) and the well-established hadith position on the topic. Fixed (`tool/fix_bayhaqi_14109_translation.dart`), but this book (and the other AI-tagged content across all 13 hadithunlocked-sourced books — see each book's own `aiTagged` count in `import_hadithunlocked.dart`'s run output) has **not** been systematically audited for this class of error; only this one flagged instance is known-fixed. See **[`../DUPLICATE_HADITH_INVESTIGATION.md`](../DUPLICATE_HADITH_INVESTIGATION.md)** for the first three. |
