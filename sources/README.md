...
|---|---|---|---|
| **amrayn.com** | ✅ `sources/amrayn.com/scripts/scrape_amrayn.dart` | ✅ `sources/amrayn.com/runs/` | Sole independent source for `riyad_assalihin`/`aladab_almufrad`/`shamail_muhammadiyah`; second-opinion cross-check for the 6 fawaz-rebuilt books + malik/nawawi40/qudsi40/darimi. See `sources/amrayn.com/runs/*/SNAPSHOT_INFO.md` for per-run status, including the **Malik URL-scheme problem** (needs a dedicated per-chapter scraper — flat `/malik:N` doesn't exist on this site, real URLs are `/malik/{chapter}/{hadithInChapter}`). |
